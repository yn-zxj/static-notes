...
</code></pre></div><h3 id="用户变量" tabindex="-1"><a class="header-anchor" href="#用户变量" aria-hidden="true">#</a> 用户变量</h3><p>  用户变量是用户自己定义的，作为 MySQL 编码规范，MySQL 中的用户变量以一个<code>@</code>开头。根据作用范围不同，又分为<mark>会话用户变量</mark>和<mark>局部变量</mark>。</p><blockquote><p>会话用户变量：作用域和会话变量一样，只对当前连接会话有效<br> 局部变量：只在<code>BEGIN</code>和<code>END</code>语句块中有效。<mark>局部变量只能在存储过程和函数中使用</mark></p></blockquote>`,19),h=s("br",null,null,-1),b={href:"https://dev.mysql.com/doc/refman/5.7/en/user-variables.html",target:"_blank",rel:"noopener noreferrer"},S=s("br",null,null,-1),v={href:"https://dev.mysql.com/doc/refman/8.0/en/user-variables.html",target:"_blank",rel:"noopener noreferrer"},N=e(`<h4 id="用户会话变量" tabindex="-1"><a class="header-anchor" href="#用户会话变量" aria-hidden="true">#</a> 用户会话变量</h4><p><strong>变量的定义</strong>：</p><div class="language-sql" data-ext="sql"><pre class="language-sql"><code><span class="token comment"># 方式一： = 或 :=</span>
...
</code></pre></div><h4 id="会话用户变量与局部变量对比" tabindex="-1"><a class="header-anchor" href="#会话用户变量与局部变量对比" aria-hidden="true">#</a> 会话用户变量与局部变量对比</h4><table><thead><tr><th></th><th>作用域</th><th>定义位置</th><th>语法</th></tr></thead><tbody><tr><td>会话用户变量</td><td>当前会话</td><td>会话的任何地方</td><td>加符号<code>@</code>，不用指定类型</td></tr><tr><td>局部变量</td><td><code>BEGIN...END</code></td><td><code>BEGIN...END</code>第一句</td><td>一般不加<code>@</code>，需要指定类型</td></tr></tbody></table><h2 id="定义条件与处理程序" tabindex="-1"><a class="header-anchor" href="#定义条件与处理程序" aria-hidden="true">#</a> 定义条件与处理程序</h2><p>  <strong>定义条件</strong>是事先定义程序执行过程中可能遇到的问题，<strong>处理程序</strong>定义了在遇到问题时应当采取的处理方式，并且保证存储过程或函数在遇到警告或错误时能继续执行。这样可以增强存储程序处理问题的能力，避免程序异常停止运行。</p><h3 id="案例分析" tabindex="-1"><a class="header-anchor" href="#案例分析" aria-hidden="true">#</a> 案例分析</h3><p>(1) 创建一个名称为&quot;UpdateDataNoCondition&quot;的存储过程，代码如下：</p><div class="language-sql" data-ext="sql"><pre class="language-sql"><code><span class="token keyword">DELIMITER</span> <span class="token comment">//</span>
...
</code></pre></div><h3 id="案例解决" tabindex="-1"><a class="header-anchor" href="#案例解决" aria-hidden="true">#</a> 案例解决</h3><p>在存储过程中，定义处理程序，捕获 sqlstate_value 值，当遇到 mysql_error_code 值为 1048 时，执行 CONTINUE 操作，并且将@proc_value 的值设置为-1</p><div class="language-sql" data-ext="sql"><pre class="language-sql"><code><span class="token keyword">DELIMITER</span> <span class="token comment">//</span>
...
<span class="token operator">+</span><span class="token comment">------+-------------+</span>
<span class="token number">1</span> <span class="token keyword">row</span> <span class="token operator">in</span> <span class="token keyword">set</span> <span class="token punctuation">(</span><span class="token number">0.00</span> sec<span class="token punctuation">)</span>
</code></pre></div><h2 id="流程控制" tabindex="-1"><a class="header-anchor" href="#流程控制" aria-hidden="true">#</a> 流程控制</h2><p>  解决复杂问题不可能通过一个 SQL 语句完成，我们需要执行多个 SQL 操作。流程控制语句的作用就是控制存储过程中 SQL 语句的执行顺序，我们完成复杂操作必不可少的一部分。</p><p>只要是执行的程序，流程就分为三大类：<br>  ① <strong>顺序结构</strong>：程序从上往下依次执行<br>  ② <strong>分支结构</strong>：程序按条件进行选择执行，从两条或多条路径中选择一条执行<br>  ③ <strong>循环结构</strong>：程序满足一定条件下，重复执行一组语句</p><p>针对于 MySQL 的流程控制语句主要有三类(<mark>注意：只能用于存储程序</mark>)：<br>  ① <strong>条件判断语句</strong>：IF 语句和 CASE 语句<br>  ② <strong>循环语句</strong>：LOOP、WHILE 和 REPEAT 语句<br>  ③ <strong>跳转语句</strong>：ITERATE 和 LEAVE 语句</p><h3 id="分支结构-if" tabindex="-1"><a class="header-anchor" href="#分支结构-if" aria-hidden="true">#</a> 分支结构(IF)</h3><p>语法结构：</p><div class="language-sql" data-ext="sql"><pre class="language-sql"><code><span class="token keyword">IF</span> search_condition <span class="token keyword">THEN</span> statement_list
...
<span class="token operator">+</span><span class="token comment">---------------------+</span>
<span class="token number">1</span> <span class="token keyword">row</span> <span class="token operator">in</span> <span class="token keyword">set</span> <span class="token punctuation">(</span><span class="token number">0.00</span> sec<span class="token punctuation">)</span>
</code></pre></div><h3 id="分支结构-case" tabindex="-1"><a class="header-anchor" href="#分支结构-case" aria-hidden="true">#</a> 分支结构(CASE)</h3><p>语法结构：</p><div class="language-sql" data-ext="sql"><pre class="language-sql"><code><span class="token comment"># 方式一</span>
...
</code></pre></div><h3 id="循环结构-loop" tabindex="-1"><a class="header-anchor" href="#循环结构-loop" aria-hidden="true">#</a> 循环结构(LOOP)</h3><p>语法结构：</p><div class="language-sql" data-ext="sql"><pre class="language-sql"><code><span class="token punctuation">[</span>loop_label:<span class="token punctuation">]</span> <span class="token keyword">LOOP</span>
    statement_list
...
</code></pre></div><h3 id="循环结构-while" tabindex="-1"><a class="header-anchor" href="#循环结构-while" aria-hidden="true">#</a> 循环结构(WHILE)</h3><p>语法结构：</p><div class="language-sql" data-ext="sql"><pre class="language-sql"><code><span class="token punctuation">[</span>while_label:<span class="token punctuation">]</span> <span class="token keyword">WHILE</span> cycle_condition <span class="token keyword">DO</span>
    statement_list
...
<span class="token operator">+</span><span class="token comment">----+</span>
<span class="token number">1</span> <span class="token keyword">row</span> <span class="token operator">in</span> <span class="token keyword">set</span> <span class="token punctuation">(</span><span class="token number">0.01</span> sec<span class="token punctuation">)</span>
</code></pre></div><h3 id="循环结构-repeat" tabindex="-1"><a class="header-anchor" href="#循环结构-repeat" aria-hidden="true">#</a> 循环结构(REPEAT)</h3><p>语法结构：</p><div class="language-sql" data-ext="sql"><pre class="language-sql"><code><span class="token punctuation">[</span>repeat_label:<span class="token punctuation">]</span> <span class="token keyword">REPEAT</span>
    statement_list
UNTIL end_condition
...
<span class="token operator">+</span><span class="token comment">----+</span>
<span class="token number">1</span> <span class="token keyword">row</span> <span class="token operator">in</span> <span class="token keyword">set</span> <span class="token punctuation">(</span><span class="token number">0.01</span> sec<span class="token punctuation">)</span>
</code></pre></div><hr><h4 id="对比三种循环结构" tabindex="-1"><a class="header-anchor" href="#对比三种循环结构" aria-hidden="true">#</a> 对比三种循环结构</h4><table><thead><tr><th>关键字</th><th>说明</th></tr></thead><tbody><tr><td><code>LOOP</code></td><td>一般用于实现简单的&quot;死&quot;循环</td></tr><tr><td><code>WHILE</code></td><td>先判断后执行</td></tr><tr><td><code>REPEAT</code></td><td>先执行后判断，无条件至少执行一次</td></tr></tbody></table><blockquote><p>这三种循环都可以省略名称，但如果循环中添加了循环控制语句(<code>LEAVE或ITERATE</code>)则必须添加名称</p></blockquote><h3 id="跳转语句-leave" tabindex="-1"><a class="header-anchor" href="#跳转语句-leave" aria-hidden="true">#</a> 跳转语句(LEAVE)</h3><p>语法结构：</p><div class="language-sql" data-ext="sql"><pre class="language-sql"><code><span class="token keyword">LEAVE</span> label
...
<span class="token keyword">SELECT</span> <span class="token variable">@num</span><span class="token punctuation">;</span>
</code></pre></div><h3 id="跳转语句-iterate" tabindex="-1"><a class="header-anchor" href="#跳转语句-iterate" aria-hidden="true">#</a> 跳转语句(ITERATE)</h3><p>语法结构：</p><div class="language-sql" data-ext="sql"><pre class="language-sql"><code><span class="token keyword">ITERATE</span> label
...
</code></pre></div><h2 id="游标" tabindex="-1"><a class="header-anchor" href="#游标" aria-hidden="true">#</a> 游标</h2><h3 id="什么是游标" tabindex="-1"><a class="header-anchor" href="#什么是游标" aria-hidden="true">#</a> 什么是游标</h3><p>  游标又称光标，是 SQL 的一个内存工作区，由系统或用户以变量的形式定义，主要用于暂时存放受 SQL 语句影响到的所有数据。</p><h3 id="使用游标步骤" tabindex="-1"><a class="header-anchor" href="#使用游标步骤" aria-hidden="true">#</a> 使用游标步骤</h3><p>  游标必须在声明处理程序之前被声明，并且变量和条件还必须在声明游标或处理程序之前被声明。如果我们在使用游标，一般需要经历下面四个步骤。需要注意的是不同的 DBMS 中，使用游标的语法可能略有不用。</p><hr><h4 id="声明游标" tabindex="-1"><a class="header-anchor" href="#声明游标" aria-hidden="true">#</a> 声明游标</h4><div class="language-sql" data-ext="sql"><pre class="language-sql"><code><span class="token keyword">DECLARE</span> cursor_name <span class="token keyword">CURSOR</span> <span class="token keyword">FOR</span> select_statement<span class="token punctuation">;</span>
...
</code></pre></div><h3 id="小结" tabindex="-1"><a class="header-anchor" href="#小结" aria-hidden="true">#</a> 小结</h3><p>  游标是 MySQL 的一个重要的功能，为逐条读取结果集中的数据，提供了完美的解决方案。跟在应用层面实现相同的功能相比，游标可以在存储程序中使用，效率高，程序也更加简洁。但同时也会带来一些性能问题，比如在使用游标的过程中，会对数据行进行加锁，这样在业务并发量大的时候，不仅会影响业务之间的效率，还会消耗系统资源，造成内存不足，这是因为游标是在内存中进行的处理。</p><div class="hint-container tip"><p class="hint-container-title">提示</p><p>养成用完之后就关闭的习惯，这样才能提高系统的整体效率。</p></div><h2 id="补充-mysql-8-0-新特性" tabindex="-1"><a class="header-anchor" href="#补充-mysql-8-0-新特性" aria-hidden="true">#</a> 补充 MySQL 8.0 新特性</h2><h3 id="全局变量持久化" tabindex="-1"><a class="header-anchor" href="#全局变量持久化" aria-hidden="true">#</a> 全局变量持久化</h3><p>  在数据库中，全局变量可以通过<code>SET GLOBAL</code>语句来设置，但是设置的变量值只会<mark>临时生效</mark>，当数据库重启后，服务器又会从 MySQL 配置文件中读取变量的默认值。在 MySQL 8.0 中新增了<code>SET PERSIST</code>命令来进行持久化设置。使用该命令会将配置保存到数据目录下的<code>mysqld-auto.cnf</code>文件中，下次启动时会读取该文件，用其中的配置来覆盖默认配置。</p><p>例如：设置服务器的最大连接数为 1000</p><div class="language-sql" data-ext="sql"><pre class="language-sql"><code><span class="token keyword">SET</span> PERSIST <span class="token keyword">global</span> max_connections <span class="token operator">=</span> <span class="token number">1000</span> <span class="token punctuation">;</span>
